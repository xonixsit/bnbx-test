    ...
    this.withdrawForm = this.fb.group({
      ...
      amount: ['', [
        ...
        (control: AbstractControl) => {
          ...
          switch(source) {
            case 'DEPOSIT':
              maxAmount = this.regularDeposits;
              break;
            case 'REFER-INCOME':
              maxAmount = this.referralIncome;
              break;
            case 'SIGNUP-BONUS':
              maxAmount = this.bonusAmount;
              break;
            default:
              maxAmount = 0;
          }
    ...
    this.walletService.getWithdrawalLockStatus(this.token).subscribe({
      next: (response: any) => {
        if (response.balanceBreakdown) {
          ...
          this.totalTransferred = response.balanceBreakdown.totalTransferred || 0;
        }
...
formatLockDuration(lockDate: Date): string {
  ...
  withdraw() {
    ...
    switch(source) {
      case 'DEPOSIT':
        sourceBalance = this.regularDeposits;
        break;
      case 'REFER-INCOME':
        sourceBalance = this.referralIncome;
        break;
      case 'SIGNUP-BONUS':
        sourceBalance = this.bonusAmount;
        break;
    }

    if (amount > sourceBalance || amount > this.withdrawableBalance) {
      if (this.isWithdrawalLocked) {
        const remainingDays = Math.ceil((this.lockedUntil!.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
        this.toastr.error(`Withdrawals are locked for ${remainingDays} more days`, '', {
    ...
    if (this.withdrawForm.valid) {
      ...
      const withdrawData = {
        ...
        source: this.withdrawForm.value.source  // Add the    
     };
