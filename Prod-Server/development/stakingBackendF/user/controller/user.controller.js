...
module.exports.userDetails = async (request, response) => {
    try {
        ...
        const teamMembers = await getAllTeamMembers(userData._id);
        // Add current user's ID to include their deposits
        teamMembers.push(userData._id);
        ...
        // console.log(totalTeamTurnover, "totalTeamTurnover");
        ...
        const referralRewards = await TransactionModel.aggregate([
            {
                $match: {
                    ...
                    isDeleted: false,
                    $or: [
                        { transactionType: "REFER-INCOME", amount: { $gt: 0 } },
                        { transactionType: "WITHDRAW", withdrawalSource: "REFER-INCOME" }
                    ]
                }
            },
            {
                $group: {
                    ...
                    earned: {
                        $sum: {
                            $cond: [
                                { $eq: ["$transactionType", "REFER-INCOME"] },
                                "$amount",
                                0
                            ]
                        }
            ...
            {
                $project: {
                    total: { $subtract: ["$earned", "$withdrawn"] }
                }
            }
        ]);

        // Calculate bonus balance from transactions
        ...
        userDataObj.totalReferralRewardBalance = referralRewards.length > 0 ? referralRewards[0].total : 0;
        userDataObj.totalBonusBalance = bonusBalance.length > 0 ? bonusBalance[0].total : 0;
       
        
        // Calculate total staked balance from active stakes
        const totalStaked = await TransactionModel.aggregate([
            {
                $match: {
                    user: userData._id,
